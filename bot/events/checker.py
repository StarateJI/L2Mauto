...
class EventsChecker:
    ...
    async def _monitor_overweight(self, window_id: str, profile: BaseProfile):
        ...
        while profile.running:
            try:
                detected_level = None  # None = "не удалось прочитать", а не "нет перевеса"
        ...
        while profile.running:
            try:
                ...
                if len(red) > 0:
                    ...
                    profile.runtime_data.health = 0  # пустая полоска = честный 0

                # камера выключается, когда бот не в бою
    ...
    async def _monitor_scheduler_mode(self, window_id: str, profile: BaseProfile) -> None:
        # v2.1: камера-часовой режима Шедули.
        # Смотрит индикатор в энерго-режиме (центр окна, где была батарейка):
        #   - точка A (автоохота, авторская): (193, 164), цвет ~оранжевый (69, 29, 4)
        #   - точки B1/B2 (край надписи расписания — она длиннее): (130, 164), (155, 164)
        # Логика:
        #   видна любая B -> Шедуля ЖИВА
        #   видна A, но не B -> автоохота БЕЗ Шедули (слетела)
        #   ничего -> окно без автоохоты
        # v2.1: РАЗГОН 2 минуты после старта (не рвёмся раньше main_loop!)
        #       + ДВОЙНАЯ проверка: ивент только если 2 прохода подряд не увидели Шедулю
        if not getattr(profile.settings, "BUY_VIA_SCHEDULER", False):
            return  # камера включается только при галочке Шедули

        started_at = time.monotonic()
        xy_a, rgb_a = parseCBT("auto_combat_ON", profile=profile)  # авторская точка автоохоты
        if xy_a is None:
            xy_a, rgb_a = (193, 164), (69, 29, 4)
        b_points = [(130, 164), (155, 164)]  # левая часть длинной надписи расписания
        thr = 40  # широкий допуск: оттенки оранжевого пляшут
        miss_streak = 0  # v2.1: сколько проходов подряд Шедуля не видна

        # v2.1: РАЗГОН — первые 2 минуты молчим (main_loop запускает Шедулю)
        while profile.running and time.monotonic() - started_at < 120:
            await asyncio.sleep(10)

        while profile.running:
            try:
                if not await profile.energo.is_on():
                    miss_streak = 0  # окно не в энерго — не наш случай
                    await asyncio.sleep(30)
                    continue

                # серия кадров: надписи мигают — ловим проявление
                saw_b = False
                saw_a = False
                for _ in range(6):
                    for bp in b_points:
                        found_b = await profile.check_pixel(bp, rgb_a, timeout=0.4, thr=thr)
                        if found_b:
                            saw_b = True
                            break
                    found_a = await profile.check_pixel(xy_a, rgb_a, timeout=0.4, thr=thr)
                    if found_a:
                        saw_a = True
                ...
                now = time.monotonic()
                last_events = self._last_event_time.setdefault(window_id, {})
                last_time = last_events.get(MonitorType.SCHEDULER_MODE, 0)

                if saw_b:
                    # Шедуля жива — всё хорошо, сбрасываем серию промахов
                    miss_streak = 0
                    last_events[MonitorType.SCHEDULER_MODE] = now
                    await asyncio.sleep(90)
                    continue

                # Шедуля не видна в этом проходе
                miss_streak += 1
                log(f"Шедуля-часовой: проход {miss_streak} без Шедули (A={saw_a})", self.tname)

                # v2.1: ивент только при 2 подряд промахах И не чаще 10 минут
                if miss_streak >= 2 and now - last_time >= 600:
                    miss_streak = 0
                    EventsManager.send_event(window_id, {"type": MonitorType.SCHEDULER_MODE})
                    if saw_a:
                        log(f"Шедуля слетела (автоохота без расписания) — ивент в {window_id}", self.tname)
                    else:
                        log(f"Индикатор пуст (окно без автоохоты) — ивент в {window_id}", self.tname)
                    last_events[MonitorType.SCHEDULER_MODE] = now
                ...
                log(f"Monitor scheduler_mode error: {e}", self.tname)
                await asyncio.sleep(10)
    ...
    def start_monitoring(self, window_id: str, profile: BaseProfile,
        ...
        checkers = {
            ...
            MonitorType.SCHEDULER_MODE: self._monitor_scheduler_mode,  # NEW (v2.0)
        }
