...
class BaseSettings:
    ...
    BUY_VIA_SCHEDULER: bool = False # NEW: закуп через Шедулю (без беготни по НПЦ) вместо Buyer
    ...
    def __post_init__(self):
        ...
        # <- FIX: валидация сонных интервалов: мин не больше макса
        for mn, mx, name in (
            (self.MIN_SLEEP_AFTER_RIP, self.MAX_SLEEP_AFTER_RIP, "MIN/MAX_SLEEP_AFTER_RIP"),
            (self.MIN_LOW_HP_DODGE_SLEEP, self.MAX_LOW_HP_DODGE_SLEEP, "MIN/MAX_LOW_HP_DODGE_SLEEP"),
            (self.MIN_PVP_DODGE_SLEEP, self.MAX_PVP_DODGE_SLEEP, "MIN/MAX_PVP_DODGE_SLEEP"),
        ):
            if mn > mx:
                raise ValueError(f"{name}: минимум ({mn}) больше максимума ({mx}) — проверь настройки")
    ...
    def _validate_schedule(self, schedule_str: str, field_name: str) -> str:
        ...
        for time_str in schedule_str.split("|"):
            try:
                ...
                log(f"Кривое время в {field_name}: '{time_str}' — пропущено, проверь настройки", level="WARNING")  # <- FIX: раньше молча выбрасывали
        return "|".join(valid)
    ...
    def _validate_schedule_range(schedule_str: str) -> str:
        ...
        if len(parts) != 2:
            log(f"Кривой диапазон SCHEDULE_SCHEDULE: '{schedule_str}' — проигнорирован, проверь настройки", level="WARNING")  # <- FIX
            return ""
        try:
            ...
            log(f"Кривое время в SCHEDULE_SCHEDULE: '{schedule_str}' — проигнорирован, проверь настройки", level="WARNING")  # <- FIX
            return ""
    ...
    def is_schedule(self, action: str, nickname: str) -> bool:
        ...
        for scheduled_time_str in times:
            ...
            # <- FIX: окно 15 минут вместо 2, «догоняем» пропущенное
            if timedelta(0) <= now - scheduled_time < timedelta(minutes=15):
                if scheduled_time_str in ex:
                    continue

                with open(log_path, "a", encoding="utf-8") as f:
                    f.write(f"{today_str}:{action.lower()}:{scheduled_time_str}\n")
                if now - scheduled_time > timedelta(minutes=2):
                    log(f"Расписание {action} догоняем с опозданием {int((now - scheduled_time).total_seconds() // 60)} мин | {nickname}", level="WARNING")
                return True
