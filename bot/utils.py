...
def findAllWindows():
    ...
    for window in all_windows:
        ...
        if nick != "No":
            ...
            # Память окон: безымянное окно пробуем опознать по дневнику позиций
            from bot.windows_memory import try_recognize
            recognized = try_recognize(info)
            if recognized and recognized not in window_info:
                info["Nickname"] = recognized
                window_info[recognized] = info

    # Память окон: обновляем дневник «кто где стоит»
    from bot.windows_memory import remember
    remember(window_info)
...
def getProfiles(profiles_path="profiles"):
    ...
    for fold in os.listdir(profiles_path):
        ...
        for f in os.listdir(fpath):
            ...
            prof_path = os.path.join(profiles_path, fold, f)
