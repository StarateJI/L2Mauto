...
class Auction(GameAction):
    ...
    async def _one_item_cycle(self) -> str:
        ...
        # 3. Пауза 5 сек перед кликом «Отмена лота».
        # Пользователь: «там иногда вылезает инфа разная на экране и перекрывает
        # кнопку Отмена, бот подумал что нечего переставлять. Добавь таймер
        # секунд 5 — нажал Продажа, подождал 5 сек, если есть таймер T_TAB_SELL_OPEN
        # то к нему добавь ещё 5, потом Отмена».
        # Эта пауза даёт время любой вылезшей инфе/перекрытию осесть, чтобы
        # клик по «Отмена» точно попал в кнопку.
