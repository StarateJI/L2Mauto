...
class Auction(GameAction):
    ...
    async def _cancel_lot(self) -> bool:
        ...
        Сначала ждём пока кнопка 'Отмена лота' станет видимой/активной
        (до 15 сек). На экран может вылезти инфа которая перекрывает кнопку —
        бот ждёт пока она оседает, проверяя кнопку каждую секунду. Как только
        кнопка видна — кликаем. Если за 15 сек не появилась — кликаем всё
        равно (вдруг кнопка есть но check_pixel не ловит).

        Потом до 2 попыток клика с ожиданием окна подтверждения.
        """
        # ── 1. Ждём видимости кнопки «Отмена лота» (до 15 сек) ──────────────
        # Пользователь: «видит кнопку — сразу жмёт, не видит — ждёт в течение
        # 15 сек, чтобы точно попасть в окно между уведомлениями».
        BTN_CANCEL_WAIT_SEC = 15
        # Используем ZONE вокруг BTN_CANCEL_LOT для проверки (как _orange_at)
        cx, cy = BTN_CANCEL_LOT
        check_zone = (cx - 25, cy - 8, 50, 16)  # 50×16 зона вокруг кнопки
        button_visible = False
        for wait_i in range(BTN_CANCEL_WAIT_SEC):
            # Берём скрин зоны кнопки и проверяем что она не перекрыта.
            # Простой способ: считаем яркие (активные) пиксели. Если кнопка
            # перекрыта вылезшим окном — пиксели будут тёмные/чужие.
            try:
                img = self._grab(check_zone)
                # Кнопка «Отмена лота» в Lineage2M — коричнево-оранжевая.
                # Активная кнопка: много R>100, G>50, B<80 пикселей.
                b, g, r = cv2.split(img)
                active_mask = (r > 100) & (g > 50) & (b < 80)
                active_count = int(np.sum(active_mask))
                if active_count > 30:  # кнопка видна и активна
                    button_visible = True
                    if wait_i > 0:
                        log(f"Аук: кнопка Отмена стала видна через {wait_i + 1}с",
        ...
        if not button_visible:
            log(f"Аук: кнопка Отмена не видна {BTN_CANCEL_WAIT_SEC}с — "
                f"кликаю вслепую (вдруг check_pixel не ловит)",
                self.window_id, level="DEBUG")
    ...
    async def _one_item_cycle(self) -> str:
        ...
        # 3. Клик "Отмена лота" — с ожиданием видимости кнопки (до 15 сек).
        # Пользователь: «видит кнопку Отмена — сразу жмёт, не видит — ждёт
        # в течение 15 сек, чтобы точно попасть в окно между уведомлениями».
        # Раньше была тупая пауза 5 сек — теперь ждём саму кнопку.
