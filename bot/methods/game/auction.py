...
class Auction(GameAction):
    ...
    def _is_status_prodano(self) -> bool:
        ...
        try:
            ...
            # ВАЖНО: НЕ используем короткое «прода» — оно совпадает с
            # «Продано» (когда лот уже продан, надо нажать «Забрать»).
            # «Продано» ≠ «Продаётся» — продано = забрать адены, продаётся = на продаже.
            # Используем только специфичные формы:
            #   «продаётся», «продается», «продаё», «продае»
            ocr_match = any(kw in text for kw in
                            ("продаёт", "продает", "продаю", "продажа",
                             "продаё", "продае", "продаетс"))
            # Дополнительно: если OCR видит «продано» — это НЕ «Продаётся»,
            # это лот уже продан (кнопка «Забрать»). Явно возвращаем False.
            if "продано" in text:
                log(f"Аук: статус = «Продано» (лот продан) — НЕ «Продаётся», "
