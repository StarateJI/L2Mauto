...
LOT_SEARCH = (63, 283, 44, 44)   # иконка лота на вкладке Продажа (VLM: центр 85,305 размер 44×44)
...
# VLM на реальном скрине (v5.11.95): иконка в окне подтверждения на (480, 405),
# размер ~80×80.
CONFIRM_ICON = (440, 365, 80, 80)
...
# ── Тайминги (секунды) — уменьшены для скорости (юзер: 1-2 сек максимум)
T_CONFIRM_SETTLE = 1.5    # 3.0 → 1.5 — анимация окна подтверждения
T_ITEM_WINDOW = 2.0      # 4.0 → 2.0 — прогрузка окна цены после клика по предмету
T_PAGE_LOAD = 1.5        # 3.0 → 1.5 — пауза после свайпа страницы
LONG_PAUSE = 2.0         # 4.0 → 2.0 — после выставления лота
T_TAB_SELL_OPEN = 2.0    # 4.0 → 2.0 — пауза после клика по вкладке Продажа
T_AUCTION_CLOSE = 1.5    # 3.0 → 1.5 — пауза после клика по крестику (закрыть аук)
T_AFTER_ENERGY_OFF = 1.5 # 2.0 → 1.5 — пауза после выхода из энерго (до открытия меню)
T_AFTER_RESIZE = 1.0     # 1.5 → 1.0 — пауза после resize (до клика вкладка Продажа)
