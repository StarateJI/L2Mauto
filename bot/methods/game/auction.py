...
class Auction(GameAction):
    ...
    async def _cancel_lot(self) -> bool:
        ...
        Пользователь: «кнопка точно такого же цвета, серый фон и белый
        текст там и там, только текст отличается». Значит цветовой фильтр
        ловит ОБЕ кнопки одинаково — различать их по пикселям бессмысленно.

        Логика:
        - Ждём 15 сек пока кнопка (серый фон + белый текст) станет видна
        - Кликаем по BTN_CANCEL_LOT
        - Ждём окно подтверждения (если «Отмена» — появится)
        - Если окно НЕ появилось после 2 кликов — это значит кнопка
          «Забрать» (она не вызывает окно подтверждения, она сразу
          забирает адены с продажи). Возвращаем False → _one_item_cycle
          скажет 'empty' (список пуст, нечего переставлять).
        """
        # ── 1. Ждём видимости кнопки (серый фон + белый текст) ─────────────
        BTN_CANCEL_WAIT_SEC = 15
        cx, cy = BTN_CANCEL_LOT
        check_zone = (cx - 25, cy - 8, 50, 16)  # 50×16 зона вокруг кнопки
        button_visible = False
        for wait_i in range(BTN_CANCEL_WAIT_SEC):
            try:
                img = self._grab(check_zone)
                b, g, r = cv2.split(img)
                # Кнопка: серый фон (50-150) + белый текст (>180).
                # Белые пиксели на сером фоне = активная кнопка.
                white_mask = (r > 180) & (g > 180) & (b > 180)
                white_count = int(np.sum(white_mask))
                if white_count > 20:  # белый текст виден → кнопка есть
                    button_visible = True
                    if wait_i > 0:
                        log(f"Аук: кнопка (Отмена/Забрать) стала видна "
                            f"через {wait_i + 1}с", self.window_id, level="DEBUG")
                    break
            except Exception:
                pass
            if wait_i < BTN_CANCEL_WAIT_SEC - 1:
                await asyncio.sleep(1.0)

        if not button_visible:
            log(f"Аук: кнопка не видна {BTN_CANCEL_WAIT_SEC}с — "
                f"кликаю вслепую по {BTN_CANCEL_LOT}",
                self.window_id, level="DEBUG")

        # ── 2. Кликаем (до 2 попыток) ──────────────────────────────────────
        for attempt in range(1, 3):
            ...
            if self._confirm_window_visible():
                log("Аук: окно подтверждения появилось (это была «Отмена»)",
                    self.window_id)
                return True
        # Окно подтверждения НЕ появилось → это была «Забрать» (забирает
        # адены без окна подтверждения) ИЛИ список пуст (кнопка серая).
        # В обоих случаях return False → _one_item_cycle скажет 'empty'.
        log("Аук: окно подтверждения НЕ появилось (кнопка «Забрать» или "
            "список пуст)", self.window_id, level="DEBUG")
        return False
    ...
    async def _one_item_cycle(self) -> str:
        ...
        # Пользователь: «для тебя ничего, делать нужно тоже самое что делал
        # дальше, просто текст другой и всё, не вникай, не обращай внимания,
        # так же переставляй эту шмотку».
        #
        # _cancel_lot вернёт:
        # - True  → окно подтверждения появилось (это была «Отмена») → кликаем ОК
        # - False → окно подтверждения НЕ появилось → это была «Забрать»
        #   (лот уже снят, без окна подтверждения) → ПРОПУСКАЕМ клик ОК и
        #   идём сразу к поиску предмета в инвентаре.
        got_confirm = await self._cancel_lot()

        if got_confirm:
            # Была «Отмена» → окно подтверждения открыто → кликаем ОК
            await asyncio.sleep(T_CONFIRM_SETTLE)
            if not await self._click_ok_cancel():
                # Окно подтверждения не закрылось — попробуем закрыть вручную
                await self._click(*BTN_CLOSE)
                return 'error'
            log("Аук: лот снят через «Отмена» (с подтверждением)",
                self.window_id)
        else:
            # Была «Забрать» → лот уже снят, окно подтверждения не появилось.
            # Идём дальше к поиску предмета (как после «Отмена»).
            log("Аук: лот снят через «Забрать» (без подтверждения) — "
                "иду искать предмет в инвентаре",
                self.window_id)
