...
class Teleport(GameAction):
    ...
    async def safe_home(self) -> bool:
        targets = []
        for tag in ("home_scroll_button_energomode", "home_scroll_button_no_energomode"):
            xy, rgb = parseCBT(tag, profile=self.profile)
            if xy is not None and rgb is not None:  # FIX: слепые кнопки не создаём
                targets.append((xy, rgb))

        if not targets:
            log("Координаты кнопок свитка не найдены (проверь CBT/регион)", self.window_id, level="ERROR")
            self.profile.notify("error", "TP: не найдены координаты кнопки свитка")
            return False
        ...
        for task, (xy, _) in zip(tasks, targets):
            if task in done and not task.cancelled() and task.exception() is None and task.result() is True:
                for p in pending:
                    p.cancel()
                # FIX: клик с подтверждением — до 2 попыток
                for attempt in range(2):
                    clicked = await self.mouse.click(self.window_info, *xy, fast=True)
                    if clicked:
                        await asyncio.sleep(1.0)  # FIX: пауза на начало телепорта
                        return True
                log("Клик по свитку не прошёл дважды", self.window_id, level="WARNING")
                return False
    ...
    async def wait_arrived(self, need: int = 7, _lvlup_retries: int = 3) -> bool:
        ...
        if xy1 is None or rgb1 is None:  # FIX: слепая проверка = честный False
            log("Не найдены координаты залупки (проверь CBT/регион)", self.window_id, level="ERROR")
            return False
        ...
        for retry in range(_lvlup_retries + 1):
            ...
            for i in range(need):
                ...
                await asyncio.sleep(0.4)  # FIX: было 0.25 — больше воздуха при лагах
            ...
            if not lvlup:
                ...
                # FIX: одна повторная попытка проверки прилёта (лаг мог съесть голосование)
                await asyncio.sleep(2)
                teleported = await self.profile.check_pixel(
                    xy1, rgb1, timeout=DELAY_TELEPORT_TO_HOME, thr=thr,
                )
                if teleported:
                    log("Догнал: на второй проверке прилёт подтвердился", self.window_id)
                    return True
                return False
    ...
    async def to_random_spot(self, from_: int = 1, to_: int = 4, fast: bool = True) -> bool:
        ...
        debug = 1  # FIX: скрины провалов в ТГ включены (автор писал их, но выключил)
        xy_open, rgb_open = parseCBT("spot_teleport_window_opened", profile=self.profile)

        for i, key in enumerate(steps):
            xy, rgb = parseCBT(key, profile=self.profile)
            if xy is None or rgb is None:  # FIX: слепая кнопка = честный отказ со скрином
                log(f"Нет координат {key} (проверь CBT/регион)", self.window_id, level="ERROR")
                self.profile.notify("error", f"TP: нет координат кнопки {key}")
                return False
            if not await self.profile.check_pixel(xy, rgb, timeout=4):  # FIX: 3 -> 4 сек
                log(f"Не нашел {key} за 4 сек", self.window_id)
                if debug:
                    self.profile.notify_screenshot(f"Не нашел {key} за 4 сек")
                return False
            x, y = xy
            ...
            if i == 0 and xy_open is not None:
                if not await self.profile.check_pixel(xy_open, rgb_open, timeout=3):  # FIX: 2 -> 3 сек
                    if debug:
            ...
            await asyncio.sleep(random.uniform(0.3, 0.6))  # FIX: 0.2-0.5 -> 0.3-0.6
