...
class PvPDodge(EventDrivenProfile):
    ...
    def get_monitors(self) -> list:
        ...
        # NEW (v2.1): Шедуля-режим — камера-часовой вместо камер банок/сосков/смерти.
        # Игровая Шедуля сама: закупает, ресает при смерти, фармит 8 часов.
        # v2.1: при галочке ВЫКЛЮЧАЕМ и камеры закупа/данжей/аука — они лезут в окно
        # и УБИВАЮТ Шедулю (телепортом/меню). Почта и награды остаются — не убивают.
        via_scheduler = getattr(self.settings, "BUY_VIA_SCHEDULER", False)
        if via_scheduler:
            monitors.append(MonitorType.SCHEDULER_MODE)
        else:
        ...
        if not via_scheduler:
            # v2.1: закуп/данжи/аук — только без Шедули (они её убивают)
        ...
        # почта и награды — безопасны для Шедули, остаются всегда
    ...
    async def main_loop(self) -> None:
        ...
        if not hunt:
            ...
            if not rip:
                # NEW (v2.0): при Шедуле — НЕ прыгаем на спот сами (телепорт убивает Шедулю!).
                # Запускаем Шедулю — она сама отправит окно фармит.
                if getattr(self.settings, "BUY_VIA_SCHEDULER", False):
                    await self._scheduler_start_full()
                else:
    ...
    async def _menu_is_open(self) -> bool:
        # проверка «главное меню открыто?» по авторскому пикселю main_menu_opened
        xy, rgb = parseCBT("main_menu_opened", profile=self)
        if xy is None:
            return False
        return await self.check_pixel(xy, rgb, timeout=0.6, thr=20)

    async def _menu_close_reliable(self) -> bool:
        # закрыть меню С ПРОВЕРКОЙ (используется только в неудачных ветках,
        # когда меню реально открыто: вкладка не нашлась, старт не нажался)
        window_id = self.window_id
        for attempt in range(3):
            if not await self._menu_is_open():
                return True  # уже закрыто

            xy, _ = parseCBT("main_menu_gui", profile=self)
            if xy is None:
                return False
            ...
            if not await self._menu_is_open():
                log(f"Меню закрыто (попытка {attempt + 1})", window_id)
                return True

        log("Меню не закрылось за 3 попытки", window_id, level="WARNING")
        return False

    async def _sleep_window_confirm(self) -> bool:
        # усыпляем окно с подтверждением — клик по батарейке в центр,
        # проверка is_on(), одна повторная попытка, TG-жалоба если не вышло
        window_id = self.window_id
        try:
            if await self.energo.is_on():
                return True  # уже спит — не трогаем (без двойного клика!)

            # попытка 1: авторский turn_on (кнопка сна + клик в центр)
            await self.energo.turn_on()
            await asyncio.sleep(4)
            if await self.energo.is_on():
                return True

            # попытка 2: точечный клик в центр батарейки
            xy, rgb = parseCBT("energomode_center_gui", profile=self)
            if xy is not None:
                await self.mouse.click(self.window_info, xy[0], xy[1])
            ...
            log("Окно не уснуло после 2 попыток", window_id, level="WARNING")
            self.notify("error", "Окно не уснуло — глянь окно!")
            self.notify_screenshot("Окно не уснуло #важно")
            return False
        except Exception as e:
            log(f"Ошибка в _sleep_window_confirm: {e}", window_id, level="WARNING")
            return False

    async def _scheduler_start_full(self) -> bool:
        # v2.1.4 — финальная механика (по реальному поведению игры):
        # разбудить -> меню -> вкладка -> СТАРТ -> меню закрывается САМО игрой ->
        # ждём ~20 сек (окну хватит осесть; в сне Шедуля сама летает за расходкой)
        # -> усыпить с подтверждением. Всё. НИКАКИХ кликов «закрыть меню» после старта!
        # (был баг: клик по закрытому меню = открыть его заново -> окно не могло уснуть)
        window_id = self.window_id
        try:
            log("Запускаю Шедулю (полный цикл)", window_id)
            ...
            if not await sch.wait_and_click("main_menu_gui", timeout=10):
                log("Меню не открылось — Шедулю не запустил", window_id, level="WARNING")
                self.notify("error", "Шедуля: не открыл главное меню")
                self.notify_screenshot("Шедуля: не открыл главное меню")
                return False

            if not await sch.wait_and_click("schedule_menu", timeout=7):
                await self._menu_close_reliable()
                log("Вкладка расписания не найдена", window_id, level="WARNING")
                self.notify("error", "Шедуля: вкладка расписания не найдена")
                self.notify_screenshot("Шедуля: вкладка расписания не найдена")
                return False
            ...
            if not await sch.wait_and_click("schedule_start", timeout=7):
                await self._menu_close_reliable()
                log("Кнопка старта не нажалась — вероятно расписание недоступно", window_id, level="WARNING")
                self.notify("error", "Шедуля: старт не нажался (инвентарь полон / не настроено?)")
                self.notify_screenshot("Шедуля: старт не нажался #важно")
                return False

            # СТАРТ нажат — меню исчезло само. НЕ ТРОГАЕМ больше ничего!
            log("Шедуля запущена — меню закрылось само, жду разгона (~20 сек)", window_id)
            await asyncio.sleep(20)  # окну хватит осесть; в сне Шедуля сама летает за расходкой

            # теперь — сон с подтверждением
            await self._sleep_window_confirm()

            self.runtime_data.current_state = BotState.AFK
            self.notify(NotifyLevel.INFO, "Шедуля запущена (8ч): фарм, закуп и рес — на ней")
            return True
        except Exception as e:
            log(f"Ошибка запуска Шедули: {e}", window_id, level="WARNING")
            self.notify_screenshot("Запуск Шедули: шось пошло не так")
            return False
    ...
    async def respawn_buy(self):
        ...
        async with self.paused_monitors():
            # при Шедуле смерть не наш ивент (камера DEATH выкл, Шедуля ресает сама).
            # Сюда попадаем только если галочка выключена — авторская ветка.
    ...
    async def back_to_spot(self):
        ...
        # при Шедуле — не бегаем/не летаем: перезапуск Шедули, она сама на точку
        if getattr(self.settings, "BUY_VIA_SCHEDULER", False):
            await self._scheduler_start_full()
            self.events_checker.start_monitoring(window_id, self, monitors=self.get_monitors)
            return True
    ...
    async def bank_restore(self):
        ...
        async with self.paused_monitors():
            ...
            result = await self.tp.wait_arrived()
            if result:
                # при Шедуле — закуп не наша забота (расходку ведёт игра).
                # Сюда попадаем только без галочки. Авторская ветка:
    ...
    async def _handle_scheduler_mode(self):
        # обработчик ивента от камеры-часовой.
        # Шедуля слетела (8ч кончились) или окно без автоохоты — перезапускаем Шедулю.
        window_id = self.window_id
        log("Шедуля-часовой: ивент — перезапускаю Шедулю", window_id)
        async with self.paused_monitors():
            ok = await self._scheduler_start_full()
            if not ok:
                log("Перезапуск Шедули не удался — камера попробует снова через ~10 мин", window_id, level="WARNING")
    ...
    EVENT_HANDLERS = {
        ...
        MonitorType.SCHEDULER_MODE: "_handle_scheduler_mode",
    }
