...
class Dungeon(EventDrivenProfile):
    ...
    async def _blessed_land_loop(self):
        ...
        try:
            ...
            for attempt in range(10):
                ...
                if len(bright_lines) > 0:
                    ...
                    if valid:
                        ...
                        # Текст уровней ТЁМНЫЙ (яркость 30-45), порог >100
                        # видит ВСЕ уровни включая серые/недоступные.
                        # Серые Ур.78 (недоступен) тоже виден с порогом >100!
                        # Поэтому берём ПРЕДпоследнюю группу = последний ДОСТУПНЫЙ.
                        groups = []
                        ...
                        # Берём ПРЕДпоследнюю группу = последний ДОСТУПНЫЙ уровень
                        # (последняя группа может быть серым/недоступным Ур.78)
                        if len(groups) >= 2:
            ...
            # VLM подтвердил по скринам от user и по логам:
            #   Окно 400x225, стрелка на x=354 px (=89% ширины)
            #   y стрелки = y текста уровня (та же строка)
            #   Размер иконки 24x36 px — широкий, попадание в 89% надёжно.
            #
            # ВАЖНО: бот выбирает ПРЕДпоследнюю группу (Ур.75),
            # а не последнюю (Ур.78 серый/недоступный).
            arrow_x = int(ww * 0.89)
