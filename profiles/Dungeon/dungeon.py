...
class Dungeon(EventDrivenProfile):
    ...
    async def _blessed_land_loop(self):
        ...
        try:
            ...
            # energo.turn_on() делает 2 клика:
            #   1. click(13, 132) — кнопка энерго (в данже ЕСТЬ)
            #   2. click(center_x, center_y) — клик в центр (подтверждение)
            #
            # В v5.11.27 я вызывал через 4 сек — это сломало заход.
            # Причина: 4 сек мало, данж ещё не полностью загрузился,
            # персонаж не встал на точку → клик в центр сдвигает его.
            #
            # Теперь: ждём 10 сек (данж точно загрузится, персонаж встанет
            # на точку), потом energo.turn_on() — клик в центр уже безопасен
            # потому что персонаж на месте.

            log("Данги: данж запущен, жду 10 сек на полную загрузку", window_id)
            await asyncio.sleep(10)

            # Скриншот после полной загрузки (для проверки)
