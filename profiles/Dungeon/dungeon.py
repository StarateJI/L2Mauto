    ...
    async def _blessed_land_loop(self):
        ...
        try:
            ...
            for scroll_attempt in range(MAX_SCROLL_ATTEMPTS):
                ...
                # ВАЖНО: Tesseract часто искажает "земля" как:
                #   "зала", "зал", "зя", "30m", "see", "зе", "зем"
                # Поэтому ищем ВСЕ варианты — главное найти слово.
                ocr_needles = [
                    "земля",         # правильное распознавание
                    "земл",          # без окончания
                    "зем",           # короче
                    "зе",            # ещё короче
                    "зала",          # ИСКАЖЁННОЕ "земля" — было у Zakamsk!
                    "зал",           # короче искажённого
                    "зя",            # самое короткое искажение
                    "благословенн",
                    "благословен",
                    "благослов",
                    "благослове",
            ...
            # ── ДАНЖ ЗАПУЩЕН — ждём загрузки интерфейса, потом усыпляем ─
            # User: 'интерфейс в данже точно такой же как и везде,
            # кнопка энерго есть и все остальные тоже'
            #
            # Логика:
            #   1. Клик по стрелке → БЕЛЫЙ ЭКРАН (загрузка данжа 3-4 сек)
            #   2. Ждём 4 сек — интерфейс данжа загружается
            #   3. Вызываем energo.turn_on() — кнопка энерго ТЕПЕРЬ доступна
            #   4. Окно уходит в сон
            #
            # РАНЬШЕ БЫЛО 2 ОШИБКИ:
            #   v5.11.24: вызывал energo.turn_on() СРАЗУ после клика — экран
            #     ещё белый, кнопок нет → клики в пустоту
            #   v5.11.25: убрал energo.turn_on() ВООБЩЕ — бот не усыплял

            log("Данги: данж запущен, жду 4 сек на загрузку интерфейса", window_id)
