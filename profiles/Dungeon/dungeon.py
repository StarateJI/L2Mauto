...
class Dungeon(EventDrivenProfile):
    ...
    async def _blessed_land_loop(self):
        ...
        try:
            ...
            for attempt in range(10):
                ...
                if len(bright_lines) > 0:
                    ...
                    if valid:
                        ...
                        # видит только АКТИВНЫЕ уровни (белые).
                        # Серые уровни (Ур.78 — недоступен) НЕ видны.
                        # Поэтому берём ПОСЛЕДНЮЮ группу = самый высокий доступный.
                        groups = []
                        ...
                        # Берём ПОСЛЕДНЮЮ группу = последний ДОСТУПНЫЙ уровень
                        # (серые/недоступные уровни не видны — порог >100 их фильтрует)
            ...
            # VLM подтвердил по скринам от user:
            #   Стрелка на x=89% ширины (а не 68% как думал раньше!)
            #   Размер 24x36 px, иконка справа от названия уровня.
            # БЫЛО: 65% → 68% → 70% — все мимо, стрелка реально на 89%.
            arrow_x = int(ww * 0.89)
