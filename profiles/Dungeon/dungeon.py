...
class Dungeon(EventDrivenProfile):
    ...
    async def _blessed_land_loop(self):
        ...
        try:
            ...
            # ВАЖНО: НЕ вызываем energo.turn_on() — он кликает по UI:
            #   1. click(13, 132) — кнопка энерго (в данже её НЕТ)
            #   2. click(center) — клик в центр экрана (в данже = в игровую зону)
            # Эти клики двигают персонажа и ломают автоохоту!
            #
            # В данже персонаж УЖЕ в автоохоте (она включена изначально).
            # Бот должен просто ждать завершения данжа и выйти.
            ...
            # energo.turn_on() НЕ вызываем — он кликает по UI которого в данже нет
